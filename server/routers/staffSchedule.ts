        ...
        .mutation(async ({ input, ctx }) => {
            ...
            if (existing.length > 0) {
                ...
                try {
                    ...
                    await db.execute(
                        sql`INSERT INTO \`staffScheduleEntries\` (\`userId\`, \`scheduleDate\`, \`status\`, \`comment\`) VALUES (${input.userId}, ${dateStr}, ${input.status}, ${input.comment || null})`
                    );
                } catch (error: any) {
            ...
            for (const update of input.updates) {
                ...
                if (existing.length > 0) {
                    ...
                    try {
                        ...
                        // 生SQLを使用して挿入（date型カラムには文字列を直接指定）
                        await db.execute(
                            sql`INSERT INTO \`staffScheduleEntries\` (\`userId\`, \`scheduleDate\`, \`status\`, \`comment\`) VALUES (${update.userId}, ${dateStr}, ${update.status}, ${update.comment || null})`
                        );
                    } catch (error: any) {
                        console.error("[staffSchedule] INSERTエラー:", error);
                        console.error("[staffSchedule] エラー詳細:", JSON.stringify(error, null, 2));
